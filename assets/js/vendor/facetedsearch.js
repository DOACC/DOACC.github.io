...
var defaults = {
  ...
  facetContainer     : '<div class=facetsearch id=<%= obj.id %> ></div>',
  facetTitleTemplate : '<h3 class=facettitle><%= obj.title %></h3>',
  facetListContainer : '<div class=facetlist></div>',
  listItemTemplate   : '<div class=facetitem id="<%= obj.id %>"><%= name %> <span class=facetitemcount>(<%= obj.count %>)</span></div>',
  bottomContainer    : '<div class=bottomline></div>',
  orderByTemplate    : '<div class=orderby><span class="orderby-title">Sort by: </span><ul><% _.each(options, function(value, key) { %>'+
                       '<li class=orderbyitem id=orderby_<%= obj.key %>>'+
                       '<%= obj.value %> </li> <% }); %></ul></div>',
  countTemplate      : '<div class=facettotalcount><%= obj.count %> Results</div>',
  ...
  resultTemplate     : '<div class="ui fitem">' +
                       '<img src="https://raw.githubusercontent.com/DOACC/imprint/master/<%= obj.imgl %>/<%= obj.img %>">' +
                       '<h4><%= obj.name %>, <%= obj.symbol %></h4>' + 
                       '<p class="tags">' + 
                       '<% if (obj.protectionscheme) {  %><%= obj.protectionscheme %><% } %>' +
                       '<% if (obj.distributionscheme) {  %>, <%= obj.distributionscheme %><% } %>' +
                       '<% if (obj.pow) {  %>, <%= obj.pow %><% } %>' +
                       '</p>' +
                       '<p class="desc"><%= obj.expiration %></p>' +
                       '</div>'
}
