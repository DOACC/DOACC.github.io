...
$rdf.SPARQLToQuery = function(SPARQL, testMode, kb) {
	...
	if (st.subject.termType == 'symbol'
	    ...
	    if($rdf.fetcher.lookUpThing) $rdf.fetcher.lookUpThing(st.subject,"sparql:"+st.subject);
	}
	if (st.object.termType == 'symbol'
	    /*&& sf.isPending(st.object.uri)*/) {
	    //sf.requestURI(st.object.uri,"sparql:"+st.object)
	    if($rdf.fetcher.lookUpThing) $rdf.fetcher.lookUpThing(st.object,"sparql:"+st.object);
	}
    }
    //alert(q.pat);
    return q;
...
$rdf.Fetcher = function(store, timeout, async) {
    ...
    ns.doacc = $rdf.Namespace("http://purl.org/net/bel-epa/doacc#");
    ns.xml = $rdf.Namespace("http://www.w3.org/XML/1998/namespace");
...
//                dump(xhr.resource + " unparseable - text/plain not visibly XML, starts:\n" + rt.slice(0, 500)+"\n")
    ...
    **/
    this.nowOrWhenFetched = function(uri, referringTerm, userCallback) {
        ...
        // If it is 'failed', then should we try again?  I think so so an old error doesn't get stuck
        //if (sta == 'unrequested')
    ...
    this.requestURI = function(docuri, rterm, force, userCallback) { //sources_request_new
        ...
        if (rterm) {
            ...
            console.log('SF.request: ' + docuri + ' refd by ' + rterm.uri)
        }
        else {
            //$rdf.log.info('SF.request: ' + docuri + ' no referring doc')
            console.log('SF.request: ' + docuri + ' no referring doc')
        };
        ...
        */
        ...
        var onreadystatechangeFactory = function(xhr) { return function() {
            var handleResponse = function() {
                ...
                var addType = function(cla) { // add type to all redirected resources too
                    ...
                    if (loc) {
                        ...
                        console.log("web.js: kb now " + kb.serialize());
                    }
                    for (;;) {
                ...
                if (xhr.status == 200) {
                    console.log("web.js: xhr status == 200");
                    addType(ns.link('Document'));
                    var ct = xhr.headers['content-type'];
                    console.log("web.js: Content data = " + xhr.headers);
                    if (ct) {
                        console.log("web.js: Content-type detected as " + ct);
                ...
                for (var x = 0; x < sf.handlers.length; x++) {
                    if (xhr.headers['content-type'] && xhr.headers['content-type'].match(sf.handlers[x].pattern)) {
                        console.log("web.js: Finding handler for " + xhr.headers['content-type']);
                        handler = new sf.handlers[x]();
                        requestHandlers.append(sf.handlers[x].term) // FYI
                        break
                    }
                }
                console.log("web.js: Weebling ");
                var link;
                try {
            ...
            console.log("web.js: XHR " + xhr.resource.uri + ' readyState='+xhr.readyState); // @@@@ comment me out
        ...
        if (typeof tabulator != 'undefined' && tabulator.preferences.get('offlineModeUsingLocalhost')) {
            if (uri2.slice(0,7) == 'http://'  && uri2.slice(7,17) != 'localhost/') {
                ...
                console.log("Localhost kludge for offline use: actually getting <" + uri2 + ">");
            } else {
                console.log("Localhost kludge NOT USED <" + uri2 + ">");
            };
        } else {
            console.log("Localhost kludge OFF offline use: actually getting <" + uri2 + ">");
        }
        // 2014 probelm:
        ...
        try {
            ...
            console.log('Accept: ' + acceptstring)
